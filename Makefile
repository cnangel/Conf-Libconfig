# This Makefile is for the Conf::Libconfig extension to perl.
#
...
#     ABSTRACT => q[The great new Conf::Libconfig!]
...
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { ExtUtils::XSBuilder=>q[0], Test::Base=>q[0.01], Filter::Util::Call=>q[0], File::Find=>q[1], ExtUtils::MakeMaker=>q[6.42], Cwd=>q[1] }
#     VERSION => q[0.01]
...
# These definitions are from config.sh (via /usr/lib/perl5/5.10.0/i386-linux-thread-multi/Config.pm)
...
SO = so
VENDORARCHEXP = /usr/lib/perl5/vendor_perl/5.10.0/i386-linux-thread-multi
...
INSTALLARCHLIB = /usr/lib/perl5/5.10.0/i386-linux-thread-multi
DESTINSTALLARCHLIB = $(DESTDIR)$(INSTALLARCHLIB)
INSTALLSITEARCH = /usr/local/lib/perl5/site_perl/5.10.0/i386-linux-thread-multi
DESTINSTALLSITEARCH = $(DESTDIR)$(INSTALLSITEARCH)
INSTALLVENDORARCH = /usr/lib/perl5/vendor_perl/5.10.0/i386-linux-thread-multi
...
INST_LIBDIR      = $(INST_LIB)/Conf
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Conf
...
XSUBPPDIR = /usr/lib/perl5/5.10.0/ExtUtils
...
DISTNAME = Conf-Libconfig
DISTVNAME = Conf-Libconfig-0.01
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Conf/Libconfig.pm $(INST_MAN3DIR)/Conf::Libconfig.$(MAN3EXT) 
...
disttest : distdir
	...
	cd $(DISTVNAME) && RELEASE_TESTING=1 $(MAKE) test  $(PASTHRU)
...
test_dynamic :: pure_all
	$(FULLPERLRUN) "-Iinc" "-MModule::Install::ExtraTests" "-e" "Module::Install::ExtraTests::__harness('Test::Harness', 1, '', '', '', $(TEST_VERBOSE), '$(INST_LIB)', '$(INST_ARCHLIB)')" $(TEST_FILES)
...
test_static :: pure_all $(MAP_TARGET)
	./$(MAP_TARGET) "-Iinc" "-MModule::Install::ExtraTests" "-e" "Module::Install::ExtraTests::__harness('Test::Harness', 1, '', '', '', $(TEST_VERBOSE), '$(INST_LIB)', '$(INST_ARCHLIB)')" $(TEST_FILES)

testdb_static :: pure_all $(MAP_TARGET)
	PERL_DL_NONLAZY=1 ./$(MAP_TARGET) $(TESTDB_SW) "-Iinc" "-I$(INST_LIB)" "-I$(INST_ARCHLIB)" $(TEST_FILE)
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>The great new Conf::Libconfig!</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>cnangel &lt;junliang.li@alibaba-inc.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Cwd" VERSION="1,0,0,0" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="i386-linux-thread-multi-5.1" />' >> $(DISTNAME).ppd
